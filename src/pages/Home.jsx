...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const alert = (message, type) => {
      ...
      wrapper.innerHTML = [
        `<div class="alert alert-${type} alert-dismissible position-absolute w-25 start-50 content-alert" role="alert" id="note" rounded>`,
    ...
    socket.on("send-message-response", (response) => {
      ...
      if (response.length) {
        if (
          ...
          console.log(
            new Date().getTime().toLocaleString("en-US", "Asia/Jakarta")
          );
          console.log(
